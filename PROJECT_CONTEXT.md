...
> **注意：本文档不包含 GitHub Token，token 需要单独提供。**
...
⚠️ **注意**：GitHub Token 保存在本地 `~/.github_token` 文件中，不会同步到其他设备。Token 需要单独提供（可复用同一 token）。

在新设备上需要重新配置：

1. 确认已有 token（从旧设备复制，或重新生成）：`ghp_...`（请替换为你的真实 token）
2. 把 token 粘贴给 Kimi Work
3. Kimi Work 会执行：
```bash
echo 'ghp_...' > ~/.github_token && chmod 600 ~/.github_token
```
4. 然后配置 git credential：
```bash
cd LongMarch-FutureAgentOS
